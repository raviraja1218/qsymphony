#!/usr/bin/env python
"""
Step 1.1: Generate layouts with correct metadata format
"""
...
def create_chip_layout(params, layout_id):
    """Create a layout with correct metadata format"""
    
    # Create design with metadata dict (WORKS!)
...
def main():
    ...
    for i, params in enumerate(tqdm(samples, desc="Creating layouts")):
        ...
        if (i + 1) % 1000 == 0:
            ...
            df_temp.to_csv(INDEX_FILE.parent / f'index_checkpoint_{i+1}.csv', index=False)
