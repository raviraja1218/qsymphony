#!/usr/bin/env python
"""
Step 1.1: Generate layouts - with correct ground plane handling
"""
...
def create_chip_layout(params, layout_id):
    """Create a layout with correct ground plane handling"""
    
    # Create design with metadata dict
    design = designs.DesignPlanar(metadata={'name': f"Layout_{layout_id}"})
    design.overwrite_enabled = True
    
    # Ground plane might be automatic or handled differently
    # Let's check if we need to add it explicitly
    try:
        # Try different possible method names
        if hasattr(design, 'add_ground_plane'):
            design.add_ground_plane()
        elif hasattr(design, 'make_ground_plane'):
            design.make_ground_plane()
        elif hasattr(design, 'add_ground'):
            design.add_ground()
        else:
            # Ground plane might be default - log it
            print(f"Debug: No ground plane method found for layout {layout_id}")
    except Exception as e:
        print(f"Debug: Ground plane note: {e}")
...
def test_single_layout():
    ...
    try:
        ...
        # Validate (check if method exists)
