...
class LayoutDataset(Dataset):
    ...
    def process(self):
        ...
        for idx, row in tqdm(df_valid.iterrows(), total=len(df_valid)):
            # Create node features (4 nodes: transmon, capacitor, resonator, ground)
            node_features = []
            
            # Node 0: Transmon
            transmon_features = [
                row['junction_width_nm'] / 500.0,  # Normalize
            ...
            # Node 3: Ground (minimal features)
            ground_features = [0.0, 0.0, 0.0]
...
def main():
    ...
    print(f"   Node features: {dataset[0].x.shape[1]} dimensions")
