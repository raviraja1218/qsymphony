...
Fixed: All nodes have same feature dimension and proper Path handling
"""
...
PROCESSED_DIR = Path(expand_path('~/Research/Datasets/qsymphony/processed'))
...
class LayoutDataset(Dataset):
    ...
    def __init__(self, root, transform=None, pre_transform=None):
        # Convert root to Path if it's string
        self.root_path = Path(root) if isinstance(root, str) else root
        super().__init__(self.root_path, transform, pre_transform)
    ...
    def processed_file_names(self):
        return [f'data_{i:06d}.pt' for i in range(10000)]
    
    @property
    def raw_dir(self) -> str:
        return str(self.root_path / 'raw')
    
    @property
    def processed_dir(self) -> str:
        return str(self.root_path / 'processed')
    ...
    def process(self):
        ...
        for idx, row in tqdm(df_valid.iterrows(), total=len(df_valid)):
            ...
            # Save
            save_path = Path(self.processed_dir) / f'data_{idx:06d}.pt'
    ...
    def get(self, idx):
        data = torch.load(Path(self.processed_dir) / f'data_{idx:06d}.pt')
        return data
...
def main():
    ...
    # Create dataset
    dataset = LayoutDataset(str(PROCESSED_DIR))
    
    # Check if processing worked
    processed_files = list(Path(dataset.processed_dir).glob('*.pt'))
    print(f"\n✅ Generated {len(processed_files)} processed files")
