#!/usr/bin/env python
"""
Robustness sweeps - FINAL VERSION with directory creation
"""
...
def sweep_kappa(policy):
    ...
    plot_path = results_dir / 'robustness_kappa.png'
...
def sweep_T1(policy):
    ...
    plot_path = results_dir / 'robustness_T1.png'
    plt.savefig(plot_path, dpi=150)
    print(f"\n✅ Plot saved: {plot_path}")
    plt.close()
...
def sweep_nth(policy):
    ...
    nth_values = [0.1, 0.3, 0.5, 0.7, 0.9]
    results = []
    ...
    plot_path = results_dir / 'robustness_nth.png'
    plt.savefig(plot_path, dpi=150)
    print(f"\n✅ Plot saved: {plot_path}")
    plt.close()
...
def main():
    print("="*60)
    print("ROBUSTNESS SWEEPS - FINAL VERSION")
    print("="*60)
