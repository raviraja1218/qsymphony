#!/usr/bin/env python
"""
Step 3.1 & 3.2: PPO Training with LSTM - Oracle Phase
    ...
    def __init__(self, env, config, mode='oracle'):
        ...
        if mode == 'oracle':
            self.reward_fn = OracleReward(
                lambda_photon=config['reward']['lambda_photon'],
                mu_thermal=config['reward']['mu_thermal'],
                target_ent=config['reward']['entanglement_target']
            )
        else:
    ...
    def update_policy(self, advantages, returns):
        ...
        for epoch in range(self.config['epochs_per_update']):
            ...
            for i in range(n_mini_batches):
                ...
                surr2 = torch.clamp(ratio, 1 - self.config['clip_param'], 
                                    1 + self.config['clip_param']) * mb_advantages
        ...
        n_updates = n_mini_batches * self.config['epochs_per_update']
