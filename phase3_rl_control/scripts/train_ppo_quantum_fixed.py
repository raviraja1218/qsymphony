#!/usr/bin/env python
"""
PPO Training with Quantum Environment - CORRECTED dimensions
...
class PPOTrainer:
    ...
    def __init__(self, env, config, mode='oracle'):
        ...
        # Get dimensions from environment - THIS IS CRITICAL
        self.obs_dim = env.observation_space.shape[0]
        self.action_dim = env.action_space.shape[0]
        
        print(f"\n📊 Environment dimensions:")
        print(f"  Observation dim: {self.obs_dim}")  # Should be 17, not 20
        print(f"  Action dim: {self.action_dim}")
        
        # Initialize networks with CORRECT observation dimension
        self.actor_critic = PPOActorCritic(
            obs_dim=self.obs_dim,  # Use actual dimension from env
...
def main():
    ...
    print("="*60)
    print(f"PPO Training with Quantum Environment - {args.mode.upper()} Phase")
    print("="*60)
    ...
    # Train
    results = trainer.train(args.timesteps)
    ...
    results_path = Path(config['paths']['data']).expanduser() / f'training_results_quantum_{args.mode}.json'
