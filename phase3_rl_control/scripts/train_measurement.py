#!/usr/bin/env python
"""
Step 3.3: Measurement-Based Training
...
class MeasurementPPOTrainer:
    ...
    def __init__(self, env, config, oracle_model_path):
        ...
        # Load oracle weights (transfer learning)
...
def main():
    ...
    # Check if golden path exists
    golden_path = Path(config['paths']['golden_path']).expanduser()
    if not golden_path.exists():
        print("\n⚠️ Golden path not found. Running extraction script...")
        os.system(f"python {Path(__file__).parent}/extract_golden_path.py")
    
    # Create environment with measurement mode
    env = QuantumControlEnv(
        mode='measurement',
        golden_path_file=str(golden_path)
    )
