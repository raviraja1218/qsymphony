...
Connects RL agent to the QSymphony environment
"""
...
try:
    from qsymphony_env_fixed2 import QSymphonyEnv
...
class QuantumControlEnv(gym.Env):
    ...
    def reset(self, seed=None):
        ...
        self.true_state = {
            'entanglement': 0.0,
            'n_q_true': quantum_obs[10],
            'n_m_true': quantum_obs[11],
            'time': quantum_obs[12],
            'correlation': 0.0,
            'purity': 1.0,
            'phase': 0.0
    ...
    def step(self, action):
        ...
        delta_norm = action[0] * 2.0  # [-2, 2] in ω_m units
        alpha_norm = (action[1] + 1.0) / 2.0  # [0, 1] in normalized units
        ...
        # Update true state info (in real implementation, extract from environment)
        ...
        if self.mode == 'oracle':
            ...
            rl_reward = -abs(quantum_obs[10])  # Penalize photons
            info['entanglement'] = 0.0
        else:
            # Measurement reward tracks golden path
            target = self._get_target_nq(quantum_obs[12])
            rl_reward = -abs(quantum_obs[10] - target)
            info['target_nq'] = target
        ...
        self.trajectory_data.append({
            ...
            'entanglement': info.get('entanglement', 0.0)
        })
