#!/usr/bin/env python
"""
Efficient PINN training with checkpoints and progress bars
"""
...
def train_for_p(p, n_epochs=500):
    ...
    for epoch in pbar:
        ...
        for step in range(n_steps):
            # Simple but effective Hamiltonian
...
def main():
    ...
    plt.tight_layout()
    plt.savefig(results_dir / 'fidelity_efficient.png', dpi=150)
    print(f"\n✅ Plot saved: {results_dir / 'fidelity_efficient.png'}")
    
    # Save results
    with open(data_dir / 'efficient_results.json', 'w') as f:
