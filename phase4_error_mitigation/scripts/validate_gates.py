#!/usr/bin/env python
"""
Step 4.6: Validate Gate Fidelity
...
data_dir.mkdir(parents=True, exist_ok=True)  # Fixed: exist_ok=True not exist-ok=True
...
class GateValidator:
    ...
    def apply_depolarizing_noise(self, rho, p):
        ...
        paulis = [
            np.kron(X, I), np.kron(Y, I), np.kron(Z, I),
            np.kron(I, X), np.kron(I, Y), np.kron(I, Z),
