#!/usr/bin/env python
"""
PINN with CORRECT depolarizing noise implementation
"""
...
def noise_sweep():
    ...
    plt.plot(p_values, final_fidelities, 'bo-', linewidth=2, markersize=8)
    plt.plot(p_values, 1 - 4/3 * np.array(p_values), 'r--', label='Theoretical maximum')
    ...
    data = {
        'p_values': p_values,
        'fidelities': final_fidelities,
        'theoretical_max': [1 - 4/3 * p for p in p_values]
    }
    ...
    return p_values, final_fidelities
