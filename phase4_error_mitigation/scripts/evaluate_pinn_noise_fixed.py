#!/usr/bin/env python
"""
Evaluate trained PINN under different noise levels - FIXED tensor conversion
"""
...
def evaluate_pinn_noise(pinn_model, p_values, n_trials=10):  # Reduced trials for speed
    """Evaluate PINN under different noise levels"""
    depolarizing = DepolarizingChannel(p=0.0)
    dt = 1e-9
    n_steps = 50  # Reduced steps for speed
    
    results = {'p': [], 'mean_fidelity': [], 'std_fidelity': []}
    
    print("\n" + "="*60)
    print("PINN NOISE EVALUATION")
    print("="*60)
    ...
    for p in p_values:
        ...
        for trial in range(n_trials):
            ...
            rho_target = pinn_model.U_target @ rho @ pinn_model.U_target.conj().T
            # Convert to CPU and then to Python float
            fidelities.append(float(rho_target[0, 0].real.cpu()))
...
def plot_noise_results(results):
    ...
    plot_path = Path('results/phase4/figures/pinn_noise_evaluation.png')
...
def main():
    ...
    results = evaluate_pinn_noise(pinn, p_values, n_trials=10)
    
    plot_noise_results(results)
    
    # Save results
    with open('results/phase4/data/pinn_noise_results.json', 'w') as f:
