#!/usr/bin/env python
"""
Step 2.1: Implement Stochastic Master Equation (SME) - FIXED for QuTiP 4.7.5
...
try:
    import qutip as qt
    from qutip import basis, tensor, destroy, qeye, mesolve, mcsolve, smesolve
...
if not config_path.exists():
    ...
    config = {
        ...
        'paths': {
            'validation': '~/projects/qsymphony/results/phase2/validation/',
            'wigner': '~/projects/qsymphony/results/phase2/wigner_baseline/',
            'data': '~/projects/qsymphony/results/phase2/data/'
        }
    }
else:
...
if not hw_params_file.exists():
    print(f"❌ Hardware parameters not found at {hw_params_file}")
...
class SME_Solver:
    ...
    def run_single_trajectory(self, tlist=None, store_states=True):
        """Run a single SME trajectory - FIXED for QuTiP 4.7.5"""
        if tlist is None:
            # Create time list
            dt = self.config['simulation']['time_step_ns'] * 1e-9  # s
            T_total = self.config['simulation']['time_total_us'] * 1e-6  # s
        ...
        print(f"  Time step: {(tlist[1]-tlist[0])*1e9:.1f} ns")
        
        try:
            # Run stochastic master equation - FIXED arguments for QuTiP 4.7.5
            result = qt.smesolve(
                self.H, rho0, tlist, 
                c_ops=self.c_ops,
                ...
                progress_bar=True,
                method='euler'  # Use Euler method for SME
            )
            ...
            # Try fallback with mesolve (deterministic) for testing
            print("\n⚠️ Trying fallback with mesolve (deterministic)...")
            try:
                result = qt.mesolve(
                    self.H, rho0, tlist, 
                    c_ops=self.c_ops,
                ...
                print("✅ Mesolve fallback successful")
                return result
            except Exception as e2:
                print(f"❌ Mesolve fallback also failed: {e2}")
                return None
    ...
    def verify_physical(self, result):
        ...
        if hasattr(result, 'states') and result.states:
            ...
            checks['trace_conservation'] = True
            print(f"  No states stored, skipping trace check")
    ...
    def plot_verification(self, result, save_path=None):
        ...
        axes[1, 0].text(0.5, 0.5, 'Photocurrent\n(Stochastic)', 
        ...
        if hasattr(result, 'states') and result.states:
            ...
            axes[1, 1].text(0.5, 0.5, 'No state data', 
        ...
        if hasattr(result, 'states') and result.states:
            ...
            axes[1, 2].text(0.5, 0.5, 'No state data', 
