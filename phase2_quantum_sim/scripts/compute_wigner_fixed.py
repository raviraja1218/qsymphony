#!/usr/bin/env python
"""
Step 2.3: Compute Baseline Wigner Functions - FIXED JSON serialization
...
n_m_0 = float(traj_data['n_m'][idx_0])  # Convert to Python float
n_m_25 = float(traj_data['n_m'][idx_25])
n_m_50 = float(traj_data['n_m'][idx_50])
...
def verify_thermal():
    ...
    W_min_0 = float(W0.min())  # Convert to Python float
    W_min_25 = float(W25.min())
    W_min_50 = float(W50.min())
    ...
    is_thermal = bool(W_min_0 > -0.05 and W_min_25 > -0.05 and W_min_50 > -0.05)
    return is_thermal
...
# Save metadata - FIXED: Convert all numpy types to Python native types
wigner_metadata = {
    'date': datetime.now().isoformat(),
    'n_trajectories': int(len(traj_files)),  # Convert to Python int
    'trajectory_used': int(traj_data['traj_id']),  # Convert to Python int
    'times_us': [0.0, 25.0, 50.0],
    'actual_times_us': [float(times_us[idx_0]), float(times_us[idx_25]), float(times_us[idx_50])],
    'n_m_values': [n_m_0, n_m_25, n_m_50],  # Already converted to float
    'is_thermal': is_thermal,  # Already converted to Python bool
