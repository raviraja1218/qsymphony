#!/usr/bin/env python
"""
Step 2.2: Generate 1000 baseline trajectories in parallel - FIXED for state vectors
...
print("="*60)
print("STEP 2.2: Generate 1000 Baseline Trajectories (FIXED for state vectors)")
print("="*60)
...
# Initial state - FIXED: Use state vector (ket) for mcsolve
# Qubit starts in ground state
psi_q = basis(N_q, 0)

# Mechanical mode in thermal state - for mcsolve we need to sample from thermal distribution
# We'll use a random state that approximates thermal statistics
# For simplicity, we'll start in ground state and let thermalization happen
psi_m = basis(N_m, 0)  # Start mechanical mode in ground state

# Combined state vector
psi0 = tensor(psi_q, psi_m)
print(f"\n🎯 Initial state: |0,0⟩ (qubit ground, mechanical ground)")

# Monte Carlo options
mc_options = SolverOptions(store_states=True, store_final_state=True)

def run_single_trajectory(traj_id, seed_offset=0):
    """Run a single Monte Carlo trajectory using state vectors"""
    ...
    try:
        # Run Monte Carlo solver with state vector initial state
        result = qt.mcsolve(
            H, psi0, tlist_full,
            c_ops=c_ops,
        ...
        # Generate photocurrent (simulated from measurement operator)
...
def save_trajectory(data):
    ...
    with open(traj_file, 'wb') as f:
        ...
        # Convert Qobj to array for saving
        if data_copy['final_state'] is not None:
            data_copy['final_state'] = data_copy['final_state'].full()
