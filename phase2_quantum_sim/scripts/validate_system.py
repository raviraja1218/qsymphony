#!/usr/bin/env python
"""
Step 2.4: Validate System Parameters
...
print(f"  ω_q/2π = {params['wq_GHz']} GHz")
print(f"  ω_m/2π = {params['wm_MHz']} MHz")
print(f"  g₀/2π = {params['g0_MHz']} MHz")
print(f"  T₁_q = {params['T1_q_us']} μs")
print(f"  T₂*_q = {params['T2_q_us']} μs")
print(f"  T₁_m = {params['T1_m_us']} μs")
print(f"  n_th = {params['n_th']:.3f}")
...
try:
    popt, pcov = curve_fit(exp_decay, times_us[fit_mask], n_q_mean[fit_mask], 
                           p0=[n_q_mean[0], params['T1_q_us']])
    T1_fit = popt[1]
    T1_err = np.sqrt(pcov[1,1])
    
    print(f"\n📈 Fitted T₁ = {T1_fit:.2f} ± {T1_err:.2f} μs")
    print(f"   Expected T₁ = {params['T1_q_us']} μs")
    print(f"   Relative error: {abs(T1_fit - params['T1_q_us'])/params['T1_q_us']*100:.2f}%")
    
    T1_match = abs(T1_fit - params['T1_q_us']) / params['T1_q_us'] < 0.05
...
if 'popt' in locals():
    ...
    ax.plot(t_fit, exp_decay(t_fit, *popt), 'r-', linewidth=2, 
...
try:
    popt_c, pcov_c = curve_fit(exp_decay_coherence, times_us[fit_mask], 
                                ...
                                p0=[np.abs(corr_mean[0]), params['T2_q_us']])
    T2_fit = popt_c[1]
    T2_err = np.sqrt(pcov_c[1,1])
    
    print(f"\n📈 Fitted T₂* = {T2_fit:.2f} ± {T2_err:.2f} μs")
    print(f"   Expected T₂* = {params['T2_q_us']} μs")
    print(f"   Relative error: {abs(T2_fit - params['T2_q_us'])/params['T2_q_us']*100:.2f}%")
    
    T2_match = abs(T2_fit - params['T2_q_us']) / params['T2_q_us'] < 0.05
...
if 'popt_c' in locals():
    t_fit = np.linspace(0, times_us[-1], 100)
    ax.plot(t_fit, exp_decay_coherence(t_fit, *popt_c), 'r-', linewidth=2,
...
try:
    popt_m, pcov_m = curve_fit(mech_thermalization, times_us[fit_mask], n_m_mean[fit_mask],
                               p0=[params['n_th'], params['T1_m_us']])
    T1_m_fit = popt_m[1]
    T1_m_err = np.sqrt(pcov_m[1,1])
    n_th_fit = popt_m[0]
    
    print(f"\n📈 Fitted T₁_m = {T1_m_fit:.2f} ± {T1_m_err:.2f} μs")
    print(f"   Expected T₁_m = {params['T1_m_us']} μs")
    print(f"   Relative error: {abs(T1_m_fit - params['T1_m_us'])/params['T1_m_us']*100:.2f}%")
    print(f"   Fitted n_th = {n_th_fit:.3f} (expected {params['n_th']:.3f})")
    
    T1_m_match = abs(T1_m_fit - params['T1_m_us']) / params['T1_m_us'] < 0.05
...
if 'popt_m' in locals():
    t_fit = np.linspace(0, times_us[-1], 100)
    ax.plot(t_fit, mech_thermalization(t_fit, *popt_m), 'r-', linewidth=2,
            label=f'Fit: T₁_m = {T1_m_fit:.1f} μs')
ax.axhline(y=params['n_th'], color='g', linestyle='--', alpha=0.5, 
           label=f'n_th = {params["n_th"]:.3f}')
ax.set_xlabel('Time (μs)', fontsize=12)
...
validation_results = {
    ...
    'parameters': params,
    'qubit_T1': {
        'expected_us': params['T1_q_us'],
        'fitted_us': float(T1_fit) if 'T1_fit' in locals() else None,
        'error_percent': float(abs(T1_fit - params['T1_q_us'])/params['T1_q_us']*100) if 'T1_fit' in locals() else None,
        'within_5_percent': bool(T1_match) if 'T1_match' in locals() else False
    },
    'qubit_T2': {
        'expected_us': params['T2_q_us'],
        'fitted_us': float(T2_fit) if 'T2_fit' in locals() else None,
        'error_percent': float(abs(T2_fit - params['T2_q_us'])/params['T2_q_us']*100) if 'T2_fit' in locals() else None,
        'within_5_percent': bool(T2_match) if 'T2_match' in locals() else False
    },
    'mechanical_T1': {
        'expected_us': params['T1_m_us'],
        'fitted_us': float(T1_m_fit) if 'T1_m_fit' in locals() else None,
        'error_percent': float(abs(T1_m_fit - params['T1_m_us'])/params['T1_m_us']*100) if 'T1_m_fit' in locals() else None,
    ...
    'overall_status': 'PASS' if (T1_match and T2_match and T1_m_match and 
                                  stability_pass and max_diff_q < 0.1 and max_diff_m < 0.1) else 'FAIL'
}
