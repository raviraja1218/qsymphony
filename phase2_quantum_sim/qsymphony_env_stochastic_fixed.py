...
class StochasticQuantumEnv(gym.Env):
    def __init__(self, seed=None):
        ...
        # Stochastic solver options - FIXED for QuTiP 4.7.5
    ...
    def step(self, action):
        ...
        try:
            # Stochastic evolution - FIXED: removed store_measurement option
            result = qt.smesolve(
                H, self.psi, tlist,
                c_ops=c_ops,
                sc_ops=sc_ops,
            ...
            if hasattr(result, 'measurement') and result.measurement:
                ...
                # Fallback if measurement not stored
            ...
            n_q_val = float(qt.expect(self.n_q, self.psi))
            n_m_val = float(qt.expect(self.n_m, self.psi))
