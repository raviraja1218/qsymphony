...
class StochasticQuantumEnv(gym.Env):
    ...
    def _compute_entanglement(self, psi):
        ...
        # For |0,0⟩ state, E_N = 0
        # For Bell state, E_N = 1.0
        # We'll use a simple proxy based on populations
        if psi.isket:
            # Check if it's approximately a Bell state
            # |Φ+⟩ = (|00⟩ + |11⟩)/√2
            amplitudes = psi.full().flatten()
            if len(amplitudes) >= 4:
                # Look for Bell state signature
                p00 = abs(amplitudes[0])**2
                p11 = abs(amplitudes[-1])**2
