...
Clean version with proper imports and syntax
"""
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        print(f"Step {i+1:2d}: E_N = {info['E_N']:.6f}")
    
    print(f"\n📊 Statistics:")
    print(f"  Mean E_N = {np.mean(E_N_values):.6f}")
    print(f"  Std E_N  = {np.std(E_N_values):.6f}")
    print(f"  Max E_N  = {np.max(E_N_values):.6f}")
    print("\n✅ Environment ready!")
